...
// Установка размеров секций сплиттера
void MainWindow::setSplitSize()
{
    QList<int> sz;  // список размеров секций
    // узнаем высоту центрального виджета
    int hs = ui->centralWidget->size().height();
    hs=floor(hs/4); // размер одной доли (25%)
    sz.append(hs);  // верхний график будет занимать 25%,
    sz.append(2*hs);// основной - 50%, нижний - 25%
    sz.append(hs);  // от высоты центрального виджета
    // устанавливаем размеры дочерних элементов сплиттера
